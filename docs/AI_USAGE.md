...
For public repo demos, use either:

- `claude` if you have the local Claude CLI logged in.
- `codex` if you have Codex / OpenAI OAuth logged in.

Use `openai` only when you want to provide an official OpenAI API key.

For Claude Code, Codex, OpenClaw, Hermes Agent, or any other shell-capable agent,
the workflow is the same: clone this repo, run `uv sync`, choose one backend, and
run `uv run biocompute discover ...`.
...
`claude` shells out to the local `claude` CLI. `codex` reads
`~/.codex/auth.json` created by `codex login`. `openai` uses only
`OPENAI_API_KEY`. `openrouter` uses only `OPENROUTER_API_KEY`.
...
Or run the provided smoke script after choosing a backend:
...
Keep `archive/runs/` and local databases uncommitted.
